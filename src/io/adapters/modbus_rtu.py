"""Modbus RTU (serial) adapter backed by ``pymodbus.client.AsyncModbusSerialClient``.
...
Notes
-----
* pymodbus wraps pyserial internally via ``loop.run_in_executor`` — we do not
  manage a thread pool directly.
* ``AsyncModbusSerialClient.__init__`` requires a running event loop; it is
  therefore instantiated inside ``connect()``.
* Every read/write is wrapped in ``asyncio.wait_for(_, timeout=config.timeout_s)``
  to prevent slow serial ports from starving the event loop (risk #10).
* Address format is the same as :mod:`~src.io.adapters.modbus_tcp`:
  ``"coil:<n>"``, ``"discrete:<n>"``, ``"holding:<n>"``, ``"input:<n>"``.
"""
...
from src.io.types import IoEvent, ModbusRtuConfig, SignalKind, SignalSpec
...
class ModbusRtuAdapter(IoAdapter):
    """Modbus RTU serial adapter.

    Parameters
    ----------
    config:
        :class:`~src.io.types.ModbusRtuConfig` for this connection.
    """
    ...
    async def connect(self) -> None:
        ...
        except (ConnectionException, OSError, FileNotFoundError) as exc:
            self._client = None
            raise IoConnectionError(
                f"Modbus RTU connect to {self._config.device} failed: {exc}"
            ) from exc
        if not connected:
            self._client = None
            raise IoConnectionError(
                f"Modbus RTU connect to {self._config.device} failed: "
                "client.connect() returned False"
            )
        self._connected = True

    async def disconnect(self) -> None:
        """Close the serial connection (idempotent)."""
        self._connected = False
    ...
    async def read(self, signal: SignalSpec) -> IoEvent:
        """Read one signal value from the Modbus RTU device."""
        if not self._connected or self._client is None:
            raise IoNotConnected(
                f"Modbus RTU not connected to {self._config.device}"
            )
        ...
        try:
            ...
            raise IoTimeout(
                f"Modbus RTU read of {signal.address} timed out after {self._config.timeout_s} s"
            ) from exc
        except (ConnectionException, ModbusException, OSError) as exc:
            raise IoProtocolError(f"Modbus RTU read of {signal.address} failed: {exc}") from exc

        return IoEvent(
            connection=self._config.device,
            kind="value_changed",
            signal=signal.name,
            value=value,
    ...
    async def write(self, signal: SignalSpec, value: bool | int | float) -> IoEvent:
        """Write one signal value to the Modbus RTU device."""
        if not self._connected or self._client is None:
            raise IoNotConnected(
                f"Modbus RTU not connected to {self._config.device}"
            )
        ...
        try:
            ...
            raise IoTimeout(
                f"Modbus RTU write to {signal.address} timed out after {self._config.timeout_s} s"
            ) from exc
        except (ConnectionException, ModbusException, OSError) as exc:
            raise IoProtocolError(f"Modbus RTU write to {signal.address} failed: {exc}") from exc
