...
// ── WEAVE (weld oscillation presets) ────────────────────────────────────────
// Lateral profile in [-1,1] for a continuous arc-length phase (in cycles).
// All presets start at 0 on the seam centerline.
function weaveProfile(type, phase) {
  const f = phase - Math.floor(phase);
  if (type === "SINE") return Math.sin(phase * 2 * Math.PI);
  let tri;                                   // triangle starting at 0
  if (f < 0.25) tri = 4 * f;
  else if (f < 0.75) tri = 1 - 4 * (f - 0.25);
  else tri = -1 + 4 * (f - 0.75);
  if (type === "TRAPEZOID") return Math.max(-1, Math.min(1, tri * 1.8)); // flat tops
  return tri;                                // TRIANGLE / ZIGZAG
}

// Densify a polyline of picks ({point,normal}) into an oscillating weave path.
// Offsets each sample by amplitude * profile(s/wavelength) along the binormal
// B = T × N (in the surface plane, perpendicular to travel). Marks edge-dwell
// at the oscillation extremes. Returns a new pick list fed to
// generateCAMTrajectory, so the weave shows up in the FK path + velocity chart.
function applyWeave(picks, weave) {
  if (!weave || weave.type === "NONE" || !picks || picks.length < 2) return picks;
  const amp = weave.amplitude, wl = Math.max(0.002, weave.wavelength);
  const up = new THREE.Vector3(0, 1, 0);
  const out = [];
  let sAccum = 0;                            // global arc length → continuous phase
  for (let i = 0; i < picks.length - 1; i++) {
    const a = picks[i], b = picks[i + 1];
    const A = new THREE.Vector3(a.point[0], a.point[1], a.point[2]);
    const B = new THREE.Vector3(b.point[0], b.point[1], b.point[2]);
    const seg = new THREE.Vector3().subVectors(B, A);
    const segLen = seg.length();
    if (segLen < 1e-6) continue;
    const T = seg.clone().normalize();
    const N = new THREE.Vector3(a.normal[0], a.normal[1], a.normal[2]).normalize();
    let Bn = new THREE.Vector3().crossVectors(T, N);
    if (Bn.lengthSq() < 1e-9) Bn = new THREE.Vector3().crossVectors(T, up);
    Bn.normalize();
    const steps = Math.max(2, Math.ceil(segLen / (wl / 8)));
    for (let k = 0; k <= steps; k++) {
      if (i > 0 && k === 0) continue;        // skip duplicate shared vertex
      const fseg = k / steps;
      ...
      const pick = { point: [P.x, P.y, P.z], normal: a.normal.slice() };
